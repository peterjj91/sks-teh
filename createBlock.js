...
if (blockName) {
  ...
  mkdirp(dirPath, (err) => {                                           // создаем
    ...
    if (err) {
      console.error(`[Popeskul] Отмена операции: ${err}`);
    }

    // Нет ошибки, поехали!
    else {
      console.log(`[Popeskul] Создание папки ${dirPath} (если отсутствует)`);
      ...
      extensions.forEach((extention) => {
        ...
        if (extention === 'scss') {
          ...
          if (!hasThisBlock) {
            ...
            fileCreateMsg = '[Popeskul] Подключение блока добавлено в projectConfig.json';
          }
        }
        ...
        // Если нужна подпапка для картинок
          ...
          if (fileExist(imgFolder) === false) {
            mkdirp(imgFolder, (err) => {
              ...
              else console.log(`[Popeskul] Создание папки: ${imgFolder} (если отсутствует)`);
            });
          } else {
            console.log(`[Popeskul] Папка ${imgFolder} НЕ создана (уже существует) `);
          }
        }
        ...
        if (fileExist(filePath) === false && extention !== 'img') {
          fs.writeFile(filePath, fileContent, (err) => {
            if (err) {
              return console.log(`[Popeskul] Файл НЕ создан: ${err}`);
            }
            console.log(`[Popeskul] Файл создан: ${filePath}`);
            if (fileCreateMsg) {
              console.warn(fileCreateMsg);
            }
          });
        } else if (extention !== 'img') {
          console.log(`[Popeskul] Файл НЕ создан: ${filePath} (уже существует)`);
        }
      });
    }
  });
} else {
  console.log('[Popeskul] Отмена операции: не указан блок');
}
